...
/******************************************************************/
	...
	{
		$this->user_info = $this->getBrowserInfo();
		$this->ip = $this->ipCheck();
		$this->check_total_vizitatori();		
	}
/******************************************************************/	
...
/******************************************************************/
	private function check_total_vizitatori() 
	{
		global $con;
		$tmp = @getRec("SELECT COUNT(ip) as total FROM vizitatori_site WHERE ip = '".$this->ip."' AND data_ = DATE(NOW());");
		if($tmp['total'] < 1){
			$user_agent = trim($this->user_info['user_agent']);
			$browser = trim($this->user_info['browser']);
			$device = ucfirst(trim($this->user_info['device']));
			$anul = date("Y");
			@mysqli_query($con, "INSERT INTO vizitatori_site (ip,data_,browser,device,anul,user_agent) ".
			"VALUES('".$this->ip."',NOW(),'".$browser."','".$device."','".$anul."','".$user_agent."');");
		}
		$tmp = @getRec("SELECT COUNT(ip) as total FROM vizitatori_site;");				
	}	
/******************************************************************/
	public function count_total() 
	{
		global $con;
		$tmp = @getRec("SELECT COUNT(ip) as total FROM vizitatori_site LIMIT 1;");
		return ($tmp['total'] < 1 ? $this->fake_vizite : $tmp['total'] + $this->fake_vizite);
	}
/******************************************************************/
